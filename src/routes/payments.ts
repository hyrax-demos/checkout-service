...
payments.post("/refunds", async (req: AuthedRequest, res: Response) => {
  ...
  // Convert once at the input boundary; everything past this point — the
  // total check, the processor call, the DB row, the response — is in
  // integer cents, matching `Order.total` and the processor contract.
