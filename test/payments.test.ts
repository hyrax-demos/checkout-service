...
describe("payments routes", () => {
  ...
  describe("POST /refunds", () => {
    ...
    describe("passes integer cents to refundProcessor", () => {
      beforeEach(() => {
      ...
      // Each of these dollar values gives a non-integer when multiplied by
      // 100 in floating point (e.g. 0.29 * 100 === 28.999999999999996).
      it.each([
        [12.34, 1234],
        [0.29, 29],
        [19.99, 1999],
        [0.07, 7],
      ])("refunds %s dollars as %i cents", async (amountDollars, expectedCents) => {
      ...
      it("rejects a single refund above the order total, comparing in cents", async () => {
        const res = await request(app)
          .post("/refunds")
          .set("Authorization", `Bearer ${token}`)
          .send({ reference: "ord_abc", amountDollars: 50.01 });
