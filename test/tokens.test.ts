...
describe("chargeIdempotencyKey", () => {
  ...
  it("returns the same key for the same order across repeated calls", () => {
    expect(chargeIdempotencyKey("order-1")).toBe(chargeIdempotencyKey("order-1"));
  });
  ...
  it("keeps the charge_ namespace prefix", () => {
    expect(chargeIdempotencyKey("order-1").startsWith("charge_")).toBe(true);
  });
});
